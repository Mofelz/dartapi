...
class AuthScreenState extends State<AuthScreen> with WidgetsBindingObserver {
  ...
  void authLinkToEmail(String email) async {
    try {
      ActionCodeSettings acs = ActionCodeSettings(
          url: 'https://morefeels.page.link/XktS?email=$email',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color.fromARGB(179, 207, 105, 31),
      body: Center(
        child: Column(
            ...
            Form(
              ...
              child: Column(
                children: [
                  ...
                  ElevatedButton(
                    style: ElevatedButton.styleFrom(backgroundColor: Colors.orange),
                    onPressed: () => authAnonymously(), child: const Text("Log in anonymously")),
                  const Padding(padding: EdgeInsets.fromLTRB(25, 5, 25, 0)),
                  ElevatedButton(
                    style: ElevatedButton.styleFrom(backgroundColor: Colors.orange),
                    onPressed: () => authLinkToEmail(controllerEmail.text.trim()), child: const Text("Log in using the email link")),
                ],
              ),
            ),
          ],
